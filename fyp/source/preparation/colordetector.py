...
def update_product_color():
    ...
    for product in products:
        color = colorDetect('images/' + str(product[0]) + '.jpg')
        update_product(conn, product[0], color)
...
def update_product(conn, id, product):
    # insert product record
    sql = "UPDATE products SET color1 = '"+str(product[0])+"',  color2 = '"+str(product[1])+"',  color3 = '"+str(
        product[2])+"',  color4 = '"+str(product[3])+"',  color5 = '"+str(product[4])+"' WHERE id = '"+str(id)+"'"
    cur = conn.cursor()
    cur.execute(sql)
